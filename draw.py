...
def add_sphere( points, cx, cy, cz, r, step ):
    coord = generate_sphere(points, cx, cy, cz, r, step)
    for i in coord:
...
def generate_torus( points, cx, cy, cz, r0, r1, step ):
    ...
    while rot <= 1:
        cir = 0
        while cir <=1:
            x = math.cos(angle*rot) * (r0 * math.cos(angle*cir) + r1) + cx
            y = r0 * math.sin(angle*cir) + cy
            z = -math.sin(angle*rot)*(r0*math.cos(angle*cir)+ r1) + cz
            add_point(mat, x, y, z)
            cir += step
        rot += step
    return mat
...
def add_torus( points, cx, cy, cz, r0, r1, step ):
    coord = generate_torus(points, cx, cy, cz, r0, r1, step)
    for i in coord:
        add_edge(points, i[0], i[1], i[2], i[0]+1, i[1], i[2])
